...
function arDispari(){
  ...
  for(var i = 0; i < arThree.length; i++){
    if(arThree[i] % 2 !== 0){
     delete arThree[i]; //meglio usare arThree.splice(i--,1);
    }
  }
  return arThree.join(",");
}
 ...
 */
  ...
  while(count < 10){
    arFour[i].push(Math.random() * (100));
    if(!arFour.includes(arFour[count])){
      count++;
    } else {
      arFour.pop();
    }
...
let arString =  ["EPICODE", "is", "great"];

arString.splice(0, 3, arString[1].length, arString[2].length, arString[3].length);
